...
cf = fitcoef
qLin = p => Math.max(0, cf.lin_a + cf.lin_b * p)
...
Plot.plot({
  ...
  y: {domain: [0, 2700], label: "\u2191 Quantity", tickFormat: d3.format(","),
      grid: true, labelAnchor: "top"},
  color: {domain: ["Linear", "Exponential", "Sigmoid"],
          range: ["#93B4F5", "#C2410C", "#2563EB"]},
  marks: [
    // the band the evidence actually covers
    Plot.rect([{x1: cf.p_lo, x2: cf.p_hi}],
              {x1: "x1", x2: "x2", y1: 0, y2: 2700, fill: "#EBF4F0"}),
...
Push it far enough and the linear fit walks into negative quantity, which no customer has ever done. The exponential never goes negative and keeps promising buyers at prices nobody would pay. Only the sigmoid settles toward zero and stays there. All three described your evidence about equally well; they disagree entirely about the world outside it, and the decision you are about to make may well live out there.
