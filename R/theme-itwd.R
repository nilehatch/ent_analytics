...
#   moving between books must never have to relearn what a colour means, so
...
#   own ink and muted greys so figures look like they belong to these pages.
#   It replaced theme_hc(), whose grey panel and white gridlines competed with
...
theme_itwd <- function(base_size = 12, base_family = "", grid = c("y", "xy", "none")) {
  ...
  gy <- if (grid %in% c("y", "xy")) {
    ggplot2::element_line(colour = itwd_rule, linewidth = 0.3)
  ...
  ggplot2::theme_minimal(base_size = base_size, base_family = base_family) +
    ggplot2::theme(
      ...
      # axes: one baseline, no ticks, labels in muted grey
      axis.line.x        = ggplot2::element_line(colour = itwd_rule, linewidth = 0.4),
      axis.line.y        = ggplot2::element_blank(),
      axis.ticks         = ggplot2::element_blank(),
      axis.text          = ggplot2::element_text(colour = itwd_muted,
                                                 size = ggplot2::rel(0.9)),
      axis.title         = ggplot2::element_text(colour = itwd_ink,
                                                 size = ggplot2::rel(0.95)),
      axis.title.x       = ggplot2::element_text(margin = ggplot2::margin(t = 8)),
      axis.title.y       = ggplot2::element_text(margin = ggplot2::margin(r = 8)),

      # text
      plot.title         = ggplot2::element_text(colour = itwd_ink, face = "bold",
                                                 size = ggplot2::rel(1.05),
                                                 margin = ggplot2::margin(b = 6)),
      plot.subtitle      = ggplot2::element_text(colour = itwd_muted,
                                                 margin = ggplot2::margin(b = 10)),
      plot.caption       = ggplot2::element_text(colour = itwd_muted, hjust = 0,
                                                 size = ggplot2::rel(0.8)),

      # legend: below the plot, unboxed, no redundant title
      legend.position    = "bottom",
      legend.title       = ggplot2::element_blank(),
      legend.text        = ggplot2::element_text(colour = itwd_ink),
