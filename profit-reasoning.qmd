...
band <- df |> filter(Pi > 0) |> group_by(lab) |>
  summarise(lo = min(P), hi = max(P), .groups = "drop")
...
Sometimes the curve never crosses zero. Every price loses money.
...
The third reading is the one worth waiting for. Profit clears zero across a wide range of prices, the curve is not steep near your candidate price, and being somewhat wrong about several things at once still leaves you in the money.
...
::: {.checkpoint}
**Before you commit, you should be able to answer four questions.**

Which reading is this — impossible, fragile, or robust? Say it in one word, and say what in the curve tells you.

If profit exists, how wide is the band? Name the lowest and highest price that still clears zero. If those two numbers are close together, you have fragile profit regardless of how large the peak is.

Which assumption carries the decision? Change one belief at a time until profit crosses zero. Whichever one gets there first with the smallest change is the one your decision rests on.

Can you do anything about it? If yes, that is your next piece of work. If no, that is your exposure, and it needs to be accepted deliberately rather than discovered later.
:::
...
Now watch the commitment, which is the slider worth spending time on. Raising $\mathsf{f}$ shifts the whole curve down without changing its shape at all, narrowing the profitable band from both ends at once. The starting position sits deliberately on a boundary: one step down and the reading turns robust, two steps up and profit is gone entirely. A forty-thousand-dollar decision moves this venture across all three readings.
...
By this point the analysis has done what analysis can. Demand was learned from evidence rather than assumed. Costs were treated as commitments you chose. Scale was made explicit instead of imagined. Profit has been examined for whether it exists, how much error it survives, and which belief it depends on.

What is left is a judgment, and it belongs to you.
