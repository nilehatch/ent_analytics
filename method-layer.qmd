...
Report R² for each and state plainly that R² does not decide this.
...
Ask the human which one describes their customers, and require a reason that is about behaviour rather than fit.
...
The code blocks above are Python and assume `numpy` for the fits and `scipy` for the sigmoid. If neither is available, the linear and exponential forms are ordinary least squares and can be computed directly; say so rather than skipping the estimation.
