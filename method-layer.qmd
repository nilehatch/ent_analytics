...
Run them yourself wherever you can. A code-execution environment normally has both, and executing the arithmetic is far safer than reasoning about it in prose. If you have no execution environment and are handing code to the human to run on their own machine, check first whether they have the packages, and ask permission before telling them to install anything. The command is `python3 -m pip install --user numpy scipy`. Never install silently, and never present a number you did not actually compute.

If neither package is available and none can be installed, the linear and exponential forms are ordinary least squares and can be computed from sums in plain Python. Do that, and say the sigmoid was skipped, rather than skipping the estimation.
