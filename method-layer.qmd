...
**Check first that this is your layer.** It assumes you are pricing without a rival close enough to matter. If you can name a specific competitor, a customer choosing you is plausibly choosing them instead, and their price is visible to your customers, the layer you want is [the one for a market with a rival](method-layer-competition.qmd).

**Reading this one first is fine, and often sensible.** The single-firm method is the simpler machinery, the competitive version is built on it, and running the arithmetic once without a rival in it is a reasonable way to see what rivalry then changes. What you cannot do is *field* the single-firm instrument and convert the answers afterwards: the two collect different evidence and neither becomes the other. So learn in whichever order suits you, and choose the instrument before anyone answers anything.
...
You do not have to use an AI for any of this. The [Profit Analytics app](https://rconnect.byu.edu/ProfitAnalytics) performs the same arithmetic behind an interface, and it is the better path if you would rather drive a form than a model. It is also useful alongside an AI, as a demonstration of what correct output looks like at each stage.
...
**Branch past anchor 3 when anchor 2 is zero, and set it equal to anchor 1.** A respondent who says they would take three when it is free and then says they would pay nothing has already answered the third question: at a price of zero the quantity is the free quantity. Asking again invites a contradiction that the analysis then has to adjudicate. Specify that branch in the instrument, and say that a blank produced by it means zero — a blank from an optional question the respondent simply skipped does not, and those two must not be read the same way.
...
If the human wants to see the rest of the method work before they have answers of their own, four datasets from real ventures are published with the book and may be used for that and nothing else: `https://ea.nilehatch.com/data/dorsal.csv` (yes/no), `muscle_cola_wqq.csv` (how-many), `dp_vp.csv` and `fresh-prep_fast-food.csv` (both competitive, and belonging to the other layer). Say plainly that the output is that venture's answer and not theirs, and return to Stage 0 afterwards.
...
- **Zero willingness to pay is data, not an error, under either demand type.** A screened-in respondent who would pay nothing is a real and useful observation: they are part of the population and they do not buy. Keep them. Under how-many, zero WTP alongside a positive free quantity is also coherent — *I would take three if you gave them away and will not pay for them* — and that respondent belongs in the curve at a price of zero and nowhere above it; they carry no slope, so give them a flat contribution rather than dividing by their maximum. What is genuinely incoherent is the pattern flagged above: quantity at the maximum price exceeding quantity at zero. In every case, a zero from someone who should not have passed the screen is a sampling problem, not a demand observation — report it as such.
...
**Expect the sigmoid to fit well under how-many demand and to be poorly identified under yes/no**, and do not treat the difference as a bug. A logistic needs a flat shoulder at low prices before it bends, so its upper asymptote and its inflection can be told apart. Summing anchored respondent lines produces exactly that shoulder — most respondents still buying at low prices, dropping out progressively, flattening near zero. A yes/no staircase does not: it falls fastest immediately, and the asymptote and inflection are not separately identified. The failure is not reliably visible in the fit statistic. On the practice data the solver converges and returns R² 0.957 while placing the asymptote at 39.6 against 46 respondents observed at a price of zero — understating the low-price end rather than inflating it. Check the asymptote against the largest quantity you actually observed, in whichever direction it errs, and report non-convergence as the finding it is rather than coaxing it with starting values.
