...
viewof uPer = Inputs.range([1, 12], {value: 1, step: 0.5, label: "Units one buyer takes"})
...
buyersReq = qReq / uPer          // sales are not people unless uPer is 1
penReq = buyersReq / popn
fmt = d3.format(",.0f")
pct = d3.format(".2%")
...
${contrib <= 0
 ...
 : `<p style="margin:0 0 .35rem">Each sale contributes <strong>$${fmt(contrib)}</strong>.
    Covering the commitment takes <strong>${fmt(qReq)}</strong> sales${
      uPer === 1 ? `` : `, which is ${fmt(buyersReq)} buyers at ${uPer} units each`}.</p>
