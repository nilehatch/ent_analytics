...
The axis below is **quantity**, not price. That is deliberate: cost is a function of how much you make, so plotted this way your cost structure is a single straight line whose intercept is the commitment $\mathsf{f}$ and whose slope is the cost per unit $\mathsf{c}$, exactly as [the previous chapter](cost.qmd) wrote it. That line never moves in what follows. The only thing the slider changes is how many customers exist to travel along it.

```{ojs}
//| echo: false
viewof kScale = Inputs.range([1, 12], {value: 1, step: 0.5,
  label: "Population, as a multiple of your sample"})
```
...
// Sample demand Q = 2500 - 2P, so inverse demand P = (2500k - Q) / 2k
// and revenue R(Q) = Q * P(Q). Scaling multiplies the whole curve by k.
rev = q => q * (2500 * kScale - q) / (2 * kScale)
qCap = 2500 * kScale

// break-even where R(Q) = C(Q):  Q^2 - 2100kQ + 2000000k = 0
disc = 2100 * kScale * (2100 * kScale) - 4 * 2000000 * kScale
be = disc > 0
  ? [(2100 * kScale - Math.sqrt(disc)) / 2, (2100 * kScale + Math.sqrt(disc)) / 2]
  : null

qs = d3.range(0, qCap + 1, Math.max(5, qCap / 400))

Plot.plot({
  width: 700, height: 430, marginLeft: 86, marginBottom: 46, marginRight: 92,
  style: {background: "transparent", fontSize: "13px", color: "#6B7280"},
  x: {domain: [0, 31000], label: "Quantity \u2192", tickFormat: d3.format(","),
      labelAnchor: "right"},
  y: {domain: [0, 10000000], label: "\u2191 Dollars",
      tickFormat: d => `$${d3.format(".2s")(d).replace("G","B")}`,
      grid: true, labelAnchor: "top"},
  marks: [
    be ? Plot.rect([{x1: be[0], x2: Math.min(be[1], qCap)}],
          {x1: "x1", x2: "x2", y1: 0, y2: 10000000, fill: "#2F855A", fillOpacity: 0.08}) : null,
    be ? Plot.text([{x: (be[0] + Math.min(be[1], qCap)) / 2}],
          {x: "x", y: 9400000, text: () => "revenue covers cost here",
           fill: "#2F855A", fontSize: 11, textAnchor: "middle"}) : null,

    // the cost structure: ONE line, unchanged by anything the slider does
    Plot.line(d3.range(0, 31001, 250).map(q => ({q, y: cost(q)})),
              {x: "q", y: "y", stroke: "#C2410C", strokeWidth: 2.5}),
    Plot.text([{q: 30500, y: cost(30500)}], {x: "q", y: "y", dx: 6, dy: -6,
              text: () => "total cost", fill: "#C2410C", textAnchor: "start", fontSize: 12}),
    Plot.dot([{q: 0, y: Ffix}], {x: "q", y: "y", r: 4, fill: "#C2410C"}),
    Plot.text([{q: 0, y: Ffix}], {x: "q", y: "y", dx: 6, dy: -12,
              text: () => "f", fill: "#C2410C", fontSize: 12, textAnchor: "start"}),

    Plot.line(qs.map(q => ({q, y: rev(q)})),
              {x: "q", y: "y", stroke: "#2563EB", strokeWidth: 2.5}),
    Plot.text([{q: qCap * 0.5, y: rev(qCap * 0.5)}], {x: "q", y: "y", dy: -14,
              text: () => "revenue", fill: "#2563EB", fontSize: 12, textAnchor: "middle"}),
...
At ${kScale}\u00d7 your sample, the best revenue available is
<strong>$${d3.format(",.0f")(rev(1250 * kScale))}</strong> against a cost of
<strong>$${d3.format(",.0f")(cost(1250 * kScale))}</strong> at that quantity.
${be
 ? `Revenue covers cost between <strong>${d3.format(",.0f")(be[0])}</strong> and
    <strong>${d3.format(",.0f")(Math.min(be[1], qCap))}</strong> units.`
 : `<span style="color:#A33A2A">The revenue curve never reaches the cost line. No price
    works, because there are not enough customers to reach one.</span>`}
</p>`
```
