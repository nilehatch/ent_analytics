...
# The Method Layer {.unnumbered #sec-method-layer-competition}
...
**Field this instrument instead of the other one, not after it.** The two collect different evidence. The single-firm layer elicits what a customer would pay for your offering; this one asks about your offering *and* a rival's at the same time, under four price conditions, and no scenario in it ever removes the rival. That is deliberate — it is what allows substitution to be measured — and it means single-firm demand cannot be recovered from competitive data, or the reverse. Choose the instrument before you field anything, because you cannot convert one into the other afterwards.

**Reading the single-firm layer first is a different matter, and a good idea.** It is the simpler machinery and this one is built on top of it. The restriction is on fielding twice, not on learning in order.

**And this layer executes a method; it does not teach one.** If you have arrived here without reading the demand chapters, read them first. The gates below ask you to judge things — whether a curve behaves like customers, whether a sample resembles a population, what an equilibrium is a prediction about — that the chapters explain and this appendix assumes.

Choose this layer when all three are true: a specific rival can be named, a customer choosing you is plausibly choosing them instead, and their price is visible to your customers. Otherwise use [the method layer for a market without a rival](method-layer.qmd). A rival nobody is comparing you to is not yet a competitor, and modelling one costs you a longer survey for parameters that will come back near zero.
...
**Report all three and let the human choose, at the gate below.** Fit statistics do not decide it, and here they rarely disqualify anything outright: on the shipped competitive datasets the linear form scores between 0.71 and 0.96 and the sigmoid reaches 0.985 and 0.988. The sigmoid fits materially better on both, and the gap matters more than it would under a single firm, because the own-price slope propagates through *both* firms' optimisations before anyone sees a price. Report the spread in equilibrium prices across the three forms alongside the fit statistics — on the practice data the linear and sigmoid fits put the rival's equilibrium price \$2.63 apart. If any form returns a negative R², that is a disqualification rather than a ranking, but do not expect it: scored on the original quantity scale, a linear fit with an intercept cannot fall below zero on the data it was fitted to.
