...
**Transform per respondent, then sum. Do not pool the points and regress.**

This is the same principle as the single-firm method, where each respondent's three anchors give their own demand line and the lines are summed. Here each respondent's four scenarios identify their own three parameters *exactly*, with no estimation at all:

```python
def respondent_system(q_00, q_own0, q_0riv, q_ownriv, wtp_own, wtp_riv):
    # Four scenarios, three unknowns. The fourth is a check, not a fitted point.
    #   q_00      = a
    #   q_own0    = a - b*wtp_own      ->  b
    #   q_0riv    = a + d*wtp_riv      ->  d
    #   q_ownriv  = a - b*wtp_own + d*wtp_riv   <- must reproduce, within noise
    if wtp_own <= 0 or wtp_riv <= 0:
        return None
    a = q_00
    b = (q_00 - q_own0) / wtp_own
    d = (q_0riv - q_00) / wtp_riv
    check = q_ownriv - (a - b*wtp_own + d*wtp_riv)
    return {"a": a, "b": b, "d": d, "residual": check}

def system_from(respondents):
    rs = [r for r in (respondent_system(**x) for x in respondents) if r]
    return {"a": sum(r["a"] for r in rs),
...
**Pooling the four points from every respondent into one regression is wrong, and wrong in a direction that hides itself.** Respondents with a high willingness to pay are asked about high prices *and* tend to buy more, so pooled least squares confounds price variation with who was answering and attenuates both slopes toward zero. On the practice dataset shipped with this book, pooling returns `b = 0.45, d = 0.02` against the correct `b = 1.02, d = 0.23` — own-price sensitivity understated by a factor of two, substitution by a factor of eleven — and it returns an R² near 0.1, which correctly signals that something is wrong without saying what. Carried through to a decision, the two methods disagree about whether the venture makes money at all.

**Use the fourth scenario as the diagnostic.** It was not used in fitting, so its residual is a genuine out-of-sample check on whether this respondent's answers are internally consistent. Report the share of respondents reproducing it within a unit. Where most do, the linear form is doing real work. Where most do not, say so — the parameters are arithmetic either way, and their meaning is not.

**For yes/no demand there are no quantity scenarios**, and two willingness-to-pay figures are enough on their own. A respondent buys the focal product when they can afford it and it beats the rival on surplus:

```python
def q_yes_no(respondents, p_own, p_riv):
    # respondents: list of (wtp_own, wtp_riv)
    return sum(1 for wo, wr in respondents
               if wo >= p_own and (wo - p_own) >= (wr - p_riv))
```

Sweep both prices over a grid, build the quantity surface, then fit `q = a − b·p_own + d·p_riv` to that surface. Pooling is safe here because the surface is constructed at prices you chose rather than at prices each respondent named, so there is no respondent heterogeneity to confound. On the shipped yes/no practice data this recovers R² above 0.87 for both products.
