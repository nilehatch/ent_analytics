...
Below, the arrows are drawn for you and everything is live. Switch the campaign between the two kinds, and move its effectiveness and its price.
...
//| echo: false
{
  ...
  const vArr = [0,1].map(h => ({x: h - 0.36, y1: 1 - scBR[h], y2: scBR[h]}));
  const hArr = [0,1].map(s => ({y: s - 0.33, x1: 1 - hyBR[s], x2: hyBR[s]}));
  return Plot.plot({
    width: 660, height: 400, marginLeft: 128, marginTop: 46, marginBottom: 34,
    style: {background: "transparent", fontSize: "13px", color: "#1F2937"},
    x: {domain: [-0.6, 1.6], ticks: [0,1], tickFormat: d => d ? "advertises" : "does not",
        label: "Hogi Yogi →", labelAnchor: "center", axis: "top"},
    y: {domain: [-0.6, 1.6], ticks: [0,1], tickFormat: d => d ? "advertises" : "does not",
        label: "↑ Smart Cookie", labelAnchor: "center"},
    marks: [
      Plot.rect(cells, {x1: d => d.x-0.42, x2: d => d.x+0.42,
                        y1: d => d.y-0.42, y2: d => d.y+0.42,
      ...
      Plot.arrow(vArr, {x1: "x", y1: "y1", x2: "x", y2: "y2",
                        stroke: "#2563EB", strokeWidth: 2.4, headLength: 9, insetEnd: 26}),
      Plot.arrow(hArr, {x1: "x1", y1: "y", x2: "x2", y2: "y",
                        stroke: "#C2410C", strokeWidth: 2.4, headLength: 9, insetEnd: 38})
    ]
  })
}
```
...
Plot.plot({
  ...
  x: {domain: [0.25, 2.0], label: "Units the campaign adds per person →",
      labelAnchor: "center"},
  y: {domain: [0, 1.20], label: "↑ Price per person",
      tickFormat: d => `$${d3.format(".2f")(d)}`, labelAnchor: "center"},
  color: {
    domain: ["neither advertises", "Smart Cookie alone", "Hogi Yogi alone",
             "both advertise", "both — and both lose", "more than one"],
    range:  ["#EFEFEE", "#CFE0FA", "#FBDCC4", "#CCE8D6", "#F0AE99", "#DDD6FE"],
    legend: true
  },
  marks: [
...
The boundaries all slope the same way. A more effective campaign is worth more, so it stays worth buying at a higher price, and every region boundary drifts right as effectiveness rises. Nothing surprising there, and it is the reason a single threshold is not the answer to *is this worth it* — the answer is a curve.

What differs between the two campaigns is what lies at the bottom. Switch to reach and the cheap end of the map is entirely green: when advertising expands the market, both firms buying it is a good outcome, at every effectiveness. Switch to comparative and a band of salmon opens along the bottom, and that band *is* the trap. It sits at low prices, which is the corollary this chapter has already stated and the map now makes geographic: the cheaper a redistributive campaign gets, the more certainly both firms buy it and the more certainly both regret it.

Above that band, in both maps, is the region where only one firm advertises, and above that the region where nobody does. Notice which firm survives longest in each. Under reach it is Smart Cookie, because the differentiated firm captures more of any expansion and can therefore justify a higher price for it. Under comparative it is Hogi Yogi, because the firm with more to gain from poaching is the one with fewer customers to lose.

Start where the chapter left off, with the comparative campaign at ten cents. Every arrow points toward advertising, both firms land in the bottom-right, and the readout says what the numbers said: both are worse off than if neither had moved. Now raise the price. Somewhere near seventeen cents Smart Cookie's arrows flip, the trap opens, and above about sixty cents nobody advertises at all.
