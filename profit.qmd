...
**Unit economics** asks whether a single customer pays for itself, usually by setting the contribution that customer produces against what it cost to acquire them. That is a real improvement on plain margin, because it pulls acquisition cost inside the unit, and acquisition cost is exactly what margin analysis habitually forgets. It is also the cost your [response rate](executing-experiments.qmd) was already telling you about.

::: {.column-margin .def-margin}
**Unit economics** — whether one customer pays for itself, acquisition cost included. A necessary condition, and not a sufficient one.
:::

Notice what it still cannot see. Unit economics establishes that $\mathsf{p}$ exceeds $\mathsf{c}$, which is necessary and is not the question this book is asking. It is silent about $\mathsf{f}$, because $\mathsf{f}$ does not live in a unit. A venture where every customer pays for itself can still fail to cover what was committed before any of them arrived.

That is a boundary on the idea rather than a flaw in it. Unit economics grew out of businesses where fixed costs are genuinely small beside variable ones: software, marketplaces, direct-to-consumer with the manufacturing rented. In that world the binding constraint really is per-customer, and the framework is not merely useful but correct. Carry it into a business with a building, a plant, a clinic, or a regulatory approval to obtain, and it will report cheerfully that things are fine while the commitment goes uncovered.

<!-- NWH (16 Aug 2026) reports confidence that focusing on unit economics is
     associated with higher probability of success and of unicorn outcomes,
     citing Eisenhardt and former students at Stanford. No such reference is in
     references.bib and I could not identify the specific paper, so the
     empirical claim is deliberately NOT made here. Supply the cite and it
     belongs in a .for-curious alongside this section. -->

So it is a screen rather than a verdict. It rules out the ventures that lose money on every sale, which is worth doing early and cheaply. What it cannot tell you is whether enough sales exist, which is what the last two chapters were for.
