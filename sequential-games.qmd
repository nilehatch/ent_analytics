...
seg1 <- tibble(x = 0, y = 0, xend = 1.1, yend = c(1.0, -1.0))
seg2 <- tibble(x = 1.1, y = rep(c(1.0, -1.0), each = 2),
               xend = 2.2, yend = ys, ok = chosen)
...
ggplot() +
  ...
  geom_segment(data = seg2, aes(x, y, xend = xend, yend = yend, alpha = ok),
               color = itwd_ink, linewidth = 0.9) +
  ...
  annotate("text", x = 1.10, y = c(1.30, -0.72), label = "Smart Cookie",
           size = 3.3, color = itwd_demand) +
  annotate("text", x = 0.52, y = c(0.72, -0.72), label = c("installs", "does not"),
           size = 3.3, color = itwd_ink) +
  annotate("text", x = 1.68, y = c(1.36, 0.64, -0.36, -1.64),
           label = c("installs", "does not", "installs", "does not"),
           size = 3.1, color = itwd_muted) +
  annotate("text", x = 2.30, y = 1.92, label = "Smart Cookie, Hogi Yogi",
           hjust = 0, size = 3.0, color = itwd_muted) +
  coord_cartesian(xlim = c(-1.0, 3.6), ylim = c(-2.05, 2.05)) +
  theme_void()
```
...
- **Run the test.** Work out your rival's best response if you sit still and if you go ahead. The same answer twice means you are buying an asset rather than changing a game, and it should clear the same bar as any other purchase.
