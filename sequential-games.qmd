...
sc_best <- function(K, hy_invests) {
  ...
  if (eqc(0.60, hyc)[3] - K > eqc(0.75, hyc)[3]) "matches" else "does not"
}
tibble::tibble(
...
At twelve cents the equipment is cheap enough that Smart Cookie wants it either way. Hogi Yogi installing it changes nothing about what Smart Cookie does. Both firms end up cheaper, nobody is deterred, and Hogi Yogi has bought a cost reduction — a perfectly good thing to buy, which did not change the game.
...
#| echo: false
#| warning: false
#| fig-cap: "The twenty-two cent case read as a sequence. Smart Cookie's chosen branch is solid and the one it rejects is faded, so the deterrence is visible: installing the equipment is what stops Smart Cookie from matching. Payoffs are net of the equipment for whichever firm bought it."
#| fig-align: center
#| fig-width: 8.6
#| fig-height: 3.9
...
ggplot() +
  ...
  annotate("text", x = 1.68, y = c(1.36, 0.64, -0.36, -1.64),
           label = c("matches", "does not", "matches", "does not"),
...
Read it backwards, which is the only way a tree can be read. In the upper branch, where Hogi Yogi has installed the equipment, Smart Cookie compares \$0.97 against \$0.99 and declines to match. In the lower branch, where Hogi Yogi has not, Smart Cookie compares \$1.34 against \$1.33 and matches. Two different answers, which is the test passing.
...
This part began by noticing that every parameter in the competition chapters was a decision somebody had already made. It ends on the decisions that cannot be revisited: chosen before revenue exists, before the prices they govern are ever charged, and judged against an equilibrium that has not happened yet. Which is what this book has been about from its first page, arriving where the stakes are highest and where the option to change your mind has been deliberately given away.
