...
Before deciding, notice how easy it is to answer badly. The campaign adds two units at \$0.75 per person, and Hogi Yogi's contribution is about fifty cents a unit, so two units is roughly a dollar of contribution against seventy-five cents of cost. Buy it. That reasoning treats the demand curve as a thing that sits still while you add units to it.

It does not sit still, because prices move once the curve does. Recompute the equilibrium with $\mathsf{a_{hy}}$ raised by two and everything else unchanged:
...
tibble::tibble(
  ...
  `Hogi Yogi profit`      = c(dollar(b$pi2, .01), dollar(a$pi2 - 0.75, .01),
                              dollar(a$pi2 - 0.75 - b$pi2, .01))
) |>
  gt() |>
  tab_options(table.font.size = 12, data_row.padding = px(5), table.width = pct(100)) |>
  cols_align("center") |>
  tab_footnote("Hogi Yogi's profit is net of the $0.75 campaign. Per person per month.",
               locations = cells_column_labels(columns = `Hogi Yogi profit`))
```

Hogi Yogi should buy the campaign. Its profit goes from \$0.96 to \$1.74, which is a gain of seventy-eight cents on a seventy-five cent investment, and the naive arithmetic was roughly right about that much.

Now look at the column it was not watching. Smart Cookie's profit rises from \$1.33 to \$3.08. Smart Cookie did not advertise, did not change its product, did not change its price deliberately, and did not spend a cent. It gained \$1.75 — more than twice what Hogi Yogi gained by paying for the campaign.

And the gap between the two firms, the thing Hogi Yogi was actually trying to close, goes from thirty-six cents to a dollar thirty-four. *It nearly quadrupled.*
...
So expanding the pool hands the differentiated firm most of the expansion. Worse for Hogi Yogi, the extra demand lets *it* raise price from \$1.02 to \$1.34 — and every cent of that rise pushes customers toward the rival, whose own best response is to raise price too, from \$1.53 to \$1.94. Both firms end up charging more and selling more, and the one with the loyal customers captures most of it.
...
It also explains why the naive arithmetic failed. *Two units at fifty cents of contribution against seventy-five cents of cost* is a first-stage calculation that never solved the second stage. It assumed the price would stay at \$1.02. The price did not stay at \$1.02, and the whole point of the exercise is that it could not have.
